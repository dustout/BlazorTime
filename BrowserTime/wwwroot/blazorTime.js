...
window.blazorTime = {
  updateAllTags: function () {
    console.log("UPDATING ALL TAGS");

    //remove old display tags
    var displayTags = document.getElementsByTagName("display-utc-to-local");
    for (var i = displayTags.length - 1; i >= 0; i--) {
      var displayTag = displayTags[i];
      displayTag.remove();
    }
    ...
    for (var i = 0; i < timeTags.length; i++) {
      ...
      var utctimeval = currentTag.innerHTML;
      var tagDate = new Date(utctimeval);

      //get format from tag
      var format = currentTag.attributes["format"];
